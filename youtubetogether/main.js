...
document.getElementById("youtubeUrl").addEventListener("keydown", async function (e) {
  if (e.key === "Enter") {
    ...
    if (videoId) {
      ...
      // ✅ NEW: Grab the category before saving
...
socket.on("watchlistUpdated", (list) => {
  ...
  folders.forEach(folder => {
    ...
    // Only render the folder if it has videos inside
    if (folderItems.length > 0) {
      ...
      // Add the videos for this folder
...
async function saveCurrentVideo(videoId, category) {
  ...
  socket.emit("addVideo", {
    ...
    category: category // Send to server
  });
...
document.getElementById("saveBtn").addEventListener("click", async () => {
  ...
  // Try to parse the URL, fallback to the global variable if the input is empty
  ...
  if (videoId) {
    ...
    alert("Please enter a valid YouTube link or search term to save.");
  }
});